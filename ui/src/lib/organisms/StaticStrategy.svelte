...
<div>
    <div class="flex justify-between items-center">
        ...
        <div class="flex flex-row items-center">
            ...
            <input class="input input-sm w-20 font-bold rounded-none"
...
<div>
    {#if selected === "headers"}
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td class="p-1">
                        <input class="input input-sm w-full rounded-none" type="text" id="{idPrefix}_{index}_hk" on:input={() => addNewIfLastElement(index)} bind:value="{header.key}" placeholder="Key" />
                    </td>
                    <td>
                        <input class="input input-sm w-full rounded-none" type="text" id="{idPrefix}_{index}_hv" on:input={() => addNewIfLastElement(index)} bind:value="{header.value}" placeholder="Value"  />
                    </td>
    ...
    {#if selected === "body"}
        <div class="my-2 flex flex-row content-between gap-2">
            ...
            <div class="form-control">
                <label class="label cursor-pointer">
                    <input type="radio" bind:group={bodyType} name="{idPrefix}_body" on:click={() => addContentType("text")} value="raw" class="radio "/>
                    <span class="label-text ml-2">raw</span>
                </label>
            </div>
            {#if bodyType === "raw"}
                <div>
                    <select class="select select-bordered select-sm rounded-box" bind:value={bodyRaw} on:change={(e) => addContentType(e.target?.value)} name="{idPrefix}_body_raw" id="{idPrefix}_body-raw_preset">
